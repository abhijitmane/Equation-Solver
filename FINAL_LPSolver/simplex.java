...
public class simplex
{
    ...
    String tmp,tk;
    static int i,j,artCnt=0,TnoV=0,noV=0,noE=0,lineNo=0,errorFlg=0,objFlg=0,minFlg=0,uFlg=0,iFlg =0,cFlg=0;
	...
	while(true)
		...
		int comb;
		comb = fact(noE+noV)/(fact(noE)*fact(noV));
		if(comb < iteration)
		{
		    cFlg = 1;
		    data += "\n\n *There is Cycle... No Solution";
		    return;
		}
	...
	try
		...
		while((line = br.readLine()) != null) 
			...
			//System.out.println("*********************************** ln-2:"+lower.charAt(lower.length()-2));
			if(declFlg == 0)
			    {
			...
			else if(lower.indexOf(",")!= -1)//||(lower.endsWith("0")&&(Character.isDigit(lower.charAt(lower.length()-2)) == false)))
			    {
				consFlg = 1;
